...
public class RouteLocatorBuilder {
	...
	public static class Builder {
		...
		/**
		 * 这是一个重 xxxSpec => AsyncBuilder 的过程
		 * xxxSpec 封装了 AsyncBuilder 在各种操作符直接配置 到 AsyncBuilder 然后返回 AsyncBuilder 元信息包含了
		 * filter 和 Predicate 和 uri[允许那些hostname] 等元信息
		 */
		public Builder route(Function<PredicateSpec, Route.AsyncBuilder> fn) {
			// 返回的AsyncBuilder 封装了 uri filters route匹配策略等
		 ...
		 *  builder的时候构建出事件流模式
		 *  但是这种构造 为什么我感觉会很消耗性能呢？ 因为 每次subscribe 都要重新走map func
		 */
